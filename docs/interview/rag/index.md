...
参数最好用实验闭环来讲，而不是只背经验值：

| 实验项 | 起始值 | 观察指标 | 调整方向 |
| --- | --- | --- | --- |
| child chunk | 512 / 768 / 1024 token | hit rate、MRR、噪声率 | 召回断裂就加大，主题混杂就减小 |
| overlap | 64 / 128 / 192 token | 边界问题召回、重复 chunk 比例 | 边界漏召回就加大，重复污染就减小 |
| dense:sparse | 0.7:0.3、0.5:0.5、RRF | 编号类问题、语义类问题分组表现 | 关键词强就提高 sparse，语义强就提高 dense |
| rough topK | 20 / 50 / 100 | rerank 前召回率、延迟 | 漏召回就加大，延迟高就减小 |
| rerank topN | 5 / 10 / 20 | faithfulness、context token 成本 | 答案缺上下文就加大，噪声多就减小 |

面试里可以说：先用经验值冷启动，再用固定 eval set 做 ablation。只要能讲出“指标、样例、调参方向”，就比单纯说 768 token 更可信。
...
- `doc` 是某个候选 chunk / 文档。
- `rank_i(doc)` 是它在第 `i` 路检索结果里的排名。
- `k` 是平滑常数，常见取 60。
- 如果某个 doc 没出现在某一路结果里，那一路不加分。
...
```text
A = 1/(60+1) + 1/(60+2) = 0.03252
C = 1/(60+3) + 1/(60+3) = 0.03174
B = 1/(60+2)               = 0.01613
D = 1/(60+1)               = 0.01639
```
...
为什么 `A` 排第一？因为它同时被 BM25 和向量检索召回，而且排名都靠前。为什么 `D` 虽然是向量第一，但不一定压过 `A`？因为它只被一路召回，证据没有 `A` 稳。

追问回答：

> RRF 融合的是排名，不是原始分数。它让“多路都认为相关”的结果排得更靠前，特别适合 BM25 + 向量检索这种分数尺度不一致的混合检索场景。如果有大量标注数据，可以学习融合权重；没有足够标注时，RRF 是更稳的工程选择。
